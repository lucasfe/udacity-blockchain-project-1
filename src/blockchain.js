...
class Blockchain {
    ...
    _addBlock(block) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                reject(Error('Could not add the block to the chain'));
            }
        });
    }
    ...
    requestMessageOwnershipVerification(address) {
        return new Promise((resolve) => {
            let returnMessage = `${address}:${new Date().getTime().toString().slice(0, -3)}:starRegistry`;
    ...
    submitStar(address, message, signature, star) {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            if (!bitcoinMessage.verify(message, address, signature)) {
                reject(new Error('message is invalid'));
            }
    ...
    getStarsByWalletAddress(address) {
        let self = this;
        let stars = [];
        this.validateChain().then(retunedValues => {
            if (typeof retunedValues === 'string') {
                console.log('Chain is valid');
            }
            else {
                retunedValues.forEach(error => {
    ...
    validateChain() {
        ...
        return new Promise(async (resolve, reject) => {
            ...
            self.chain.forEach(async function (block, height) {
                ...
                let previosBlHash = null;
                if (height > 0) {
                    previosBlHash = self.chain[height - 1].hash;
                }

                if (block.previousBlockHash != previosBlHash) {
                    errorLog.push('Previous block has non consistent hash');
                }

                await block.validate().then(function (isValid) {
                    if (isValid == false) {
                        errorLog.push('Previous block is not valid');
                    }
                });
